...
a convention, or a technique from somewhere private, attribute it to the decision
itself — "maintainer decision, 2026-09-14" — rather than to the repository you read it
in. The substance transfers; the provenance does not. An entry's `source:` field is the
easiest place to get this wrong, because citing an origin is exactly what the field
asks for.
...
resolve the reference anyway, so it costs them nothing to remove and costs you a
rewrite to leave in.
